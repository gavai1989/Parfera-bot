...
async def send_home(target_message: Message):
    text = ("<b>PARFERA</b>\n\n"
            "Нишевая парфюмерия и персональный подбор.\n\n"
            "💬 <b>Просто напишите, какой аромат вы ищете.</b>\n"
            "Например: «женский сладкий до 7000», «Versace Eros 100 мл» "
            "или «что-нибудь похожее на Erba Pura».\n\n"
            "Выберите раздел:")
    ...
    if target_message.photo:
        await target_message.delete()
        return await target_message.answer(text, reply_markup=home_kb())
    return await target_message.answer(text, reply_markup=home_kb())
